...
# prep mouse data (all training)
mouse_data = pd.read_csv("oral_data/mouse.csv")
X_mouse = mouse_data[config.smilescol]
Y_mouse = mouse_data[config.labelcol]
# convert feature pandas dataframe to list for tokenization
X_mouse = X_mouse.tolist()
# convert label pandas dataframe to tensor
Y_mouse = torch.tensor(Y_mouse.tolist())
# make one hot label matrices
Y_hot_mouse = torch.zeros(Y_mouse.size(0), Y_mouse.max() + 1)
Y_hot_mouse.scatter_(1, Y_mouse.unsqueeze(1), 1)
# create CustomDataset object
mouse_dataset = CustomDataset(tokenizer, X_mouse, Y_hot_mouse, max_input_length=512, max_target_length=512)
# create Dataloader
mouse_sampler = RoundRobinBatchSampler(mouse_dataset, len_smallest_trainset)
mouse_dataloader = torch.utils.data.DataLoader(mouse_dataset, batch_sampler=mouse_sampler, worker_init_fn=seed_worker, generator=gen, shuffle=False)


# prep woman and man data
num_tasks = 2
tasks = [None] * num_tasks

data_filepaths = ["data/man.csv", "data/woman.csv"]
for task_id, filepath in enumerate(data_filepaths):
    # import data
...
for epoch in tqdm(range(config.epochs)):
    ...
    if True:
        ...
        train_mouse_running_loss = 0
        for batch in mouse_dataloader():
            ...
            train_mouse_running_loss += loss

            optimizer.zero_grad()
            train_batch_total_loss.backward()
            optimizer.step()

        train_mouse_avg_loss = train_mouse_running_loss / len(mouse_dataloader) 
        wandb.log({'mouse train loss': train_mouse_avg_loss})
    ...
    # validation
    num_test_tasks = num_tasks-1
    if True:
        val_dataloaders = [task[2] for task in tasks]
        val_dataloaders.remove("skip")
        val_preds = [[] for _ in range(num_test_tasks)]
        val_labels = [[] for _ in range(num_test_tasks)]
        val_running_losses = [0] * num_test_tasks
        val_avg_losses = [0] * num_test_tasks
        ...
        aucs = [0] * num_test_tasks
        for task_id in range(num_test_tasks):
            auc = calc_auc(val_labels[task_id], val_preds[task_id])
            aucs[task_id] = auc

        # unweighted
        auc_avg = sum(aucs) / num_test_tasks
