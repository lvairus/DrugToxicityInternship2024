...
trainstep = 0
running_loss = 0.
binacc_metric = BinaryAccuracy().to(device)
bauroc_metric = BinaryAUROC().to(device)
...
for i in tqdm(range(args.epochs)):
    ...
    with torch.no_grad():
        ...
        #r2_k = sklearn.metrics.r2_score(y_grnd_total, y_hat_total)
            binacc_metric.update(y_hat.flatten(), y_grnd)
            binacc_k = binacc_metric.compute()
            bauroc_metric.update(y_hat.flatten(), y_grnd)
            bauroc_k = bauroc_metric.compute()
        
        binacc_history.append({'epoch' : i, 'minibatch' : k, 'trainstep' : trainstep,
                                      'task' : 'tox', 'Binary Accuracy' : binacc_k})

        bauroc_history.append({'epoch' : i, 'minibatch' : k, 'trainstep' : trainstep,
                                      'task' : 'tox', 'Binary AUROC' : bauroc_k})

        binacc_list.append(binacc_k.cpu().detach().numpy())
        bauroc_list.append(bauroc_k.cpu().detach().numpy())
        trainstep += 1
        wandb.log({"val-BinaryAccuracy": binacc_k})
        wandb.log({"val-BinaryAUROC": bauroc_k})
        if bauroc_k.cpu().detach().numpy() == np.max(bauroc_list):
            torch.save({
                'epoch': i,
...
binacc_df = pd.DataFrame(binacc_history)
binacc_df.to_csv(f'binacc_{base}.csv')

bauroc_df = pd.DataFrame(bauroc_history)
bauroc_df.to_csv(f'bauroc_{base}.csv')
